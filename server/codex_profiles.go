...
func defaultDBPath() string {
	...
	defaultPath := filepath.Join(home, ".oneauthwatch", "data", "oneauthwatch.db")
...
// legacyCodexProfilesDirs returns old profile directories for migration purposes.
func legacyCodexProfilesDirs() []string {
	home, err := os.UserHomeDir()
	if err != nil {
		return nil
	}
...
func migrateCodexProfiles() {
	...
	for _, oldDir := range legacyCodexProfilesDirs() {
...
// runCodexCommand handles the `oneauthwatch codex` subcommand.
func runCodexCommand() error {
	...
	case "save":
		if len(subArgs) < 3 {
			return fmt.Errorf("usage: oneauthwatch codex profile save <name> [--auth-file <path>]")
		}
	...
	case "delete":
		if len(subArgs) < 3 {
			return fmt.Errorf("usage: oneauthwatch codex profile delete <name>")
		}
	...
	case "refresh":
		if len(subArgs) < 3 {
			return fmt.Errorf("usage: oneauthwatch codex profile refresh <name> [--auth-file <path>]")
		}
		name, authFile := parseCodexProfileArgs(subArgs[2:])
...
func printCodexHelp() error {
	...
	fmt.Println()
	fmt.Println("Usage: oneauthwatch codex profile <command> [args]")
	fmt.Println()
	...
	fmt.Println("  oneauthwatch codex profile save work       # Save current credentials as 'work'")
	fmt.Println("  oneauthwatch codex profile refresh work    # Refresh profile 'work' from current auth")
	fmt.Println("  oneauthwatch codex profile save personal   # Save current credentials as 'personal'")
	fmt.Println("  oneauthwatch codex profile list            # List all saved profiles")
	fmt.Println("  oneauthwatch codex profile delete work     # Delete the 'work' profile")
	fmt.Println()
	fmt.Println("  # Docker: import from a mounted auth file")
	fmt.Println("  oneauthwatch codex profile save Plus --auth-file /import/auth.json")
	fmt.Println("  oneauthwatch codex profile refresh Plus --auth-file /import/auth.json")
	fmt.Println()
	fmt.Println("Workflow:")
	fmt.Println("  1. Log into your first Codex account")
	fmt.Println("  2. Run: oneauthwatch codex profile save work")
	fmt.Println("  3. Log into your second Codex account")
	fmt.Println("  4. Run: oneauthwatch codex profile save personal")
	fmt.Println("  5. OneAuthWatch will poll both profiles simultaneously")
	fmt.Println()
...
func codexProfileRefresh(name, authFile string) error {
	...
	for _, p := range profiles {
		...
		if isDuplicateCodexProfile(p, refreshCreds) {
			return fmt.Errorf("account %s is already saved as profile %q.\nTo update credentials, run: oneauthwatch codex profile refresh %s", creds.AccountID, p.Name, p.Name)
		}
	}
...
func codexProfileSave(name, authFile string) error {
	...
	fmt.Println()
	fmt.Println("OneAuthWatch will poll this profile when running.")
...
func codexProfileList() error {
	...
	if len(profiles) == 0 {
		...
		fmt.Println("  2. Save profile: oneauthwatch codex profile save <name>")
		return nil
	}
...
func codexProfileStatus() error {
	...
	fmt.Println()
	fmt.Println("Profiles will be polled when OneAuthWatch is running.")
