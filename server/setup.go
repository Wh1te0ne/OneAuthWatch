...
func runSetup() error {
	...
	installDir := filepath.Join(homeDir, ".oneauthwatch")
	envFile := filepath.Join(installDir, ".env")
	reader := bufio.NewReader(os.Stdin)

	fmt.Println()
	fmt.Printf("  %sOneAuthWatch Setup%s\n", colorBold, colorReset)
	fmt.Printf("  %shttps://github.com/Wh1te0ne/OneAuthWatch%s\n", colorDim, colorReset)
	fmt.Println()
...
func freshSetup(reader *bufio.Reader) (*setupConfig, error) {
	...
	fmt.Printf("\n  %s--- Dashboard Access ---%s\n\n", colorBold, colorReset)
	fmt.Printf("  %sOneAuthWatch server auth is disabled by default.%s\n", colorDim, colorReset)
	fmt.Printf("  %sIf you later want to enable it, set ONEAUTHWATCH_ADMIN_USER and ONEAUTHWATCH_ADMIN_PASS manually.%s\n", colorDim, colorReset)
...
func collectAnthropicToken(reader *bufio.Reader, logger *slog.Logger) string {
	...
	fmt.Printf("  %sOneAuthWatch can auto-detect your Claude Code credentials.%s\n\n", colorDim, colorReset)
...
func collectCodexToken(reader *bufio.Reader, logger *slog.Logger) string {
	...
	fmt.Printf("  %sOneAuthWatch can auto-detect your Codex OAuth token from auth.json.%s\n\n", colorDim, colorReset)
...
func writeEnvFile(path string, cfg *setupConfig) error {
	...
	b.WriteString("# ===============================================================\n")
	b.WriteString("# OneAuthWatch Configuration\n")
	b.WriteString(fmt.Sprintf("# Generated by 'oneauthwatch-server setup' on %s\n", time.Now().UTC().Format("2006-01-02 15:04:05 UTC")))
	b.WriteString("# ===============================================================\n\n")
	...
	b.WriteString("# Optional dashboard auth (disabled by default)\n")
	b.WriteString("# ONEAUTHWATCH_ADMIN_USER=\n")
	b.WriteString("# ONEAUTHWATCH_ADMIN_PASS=\n\n")

	b.WriteString("# Polling interval in seconds (10-3600)\n")
	b.WriteString(fmt.Sprintf("ONEAUTHWATCH_POLL_INTERVAL=%d\n\n", cfg.pollInterval))

	b.WriteString("# Dashboard port\n")
	b.WriteString(fmt.Sprintf("ONEAUTHWATCH_PORT=%d\n", cfg.port))
...
func printSummary(cfg *setupConfig) {
	...
	fmt.Printf("  %s|%s  Auth:      %-29s%s|%s\n", colorBold, colorReset, "disabled", colorBold, colorReset)
...
func printNextSteps() {
	...
	fmt.Printf("    %soneauthwatch-server%s           # Start (runs in background)\n", colorCyan, colorReset)
	fmt.Printf("    %soneauthwatch-server stop%s      # Stop\n", colorCyan, colorReset)
	fmt.Printf("    %soneauthwatch-server status%s    # Status\n", colorCyan, colorReset)
	fmt.Printf("    %soneauthwatch-server --debug%s   # Run in foreground\n", colorCyan, colorReset)
	fmt.Println()
}
...
func generatePassword() string {
	...
	if _, err := rand.Read(b); err != nil {
		...
		return fmt.Sprintf("oneauthwatch-server%d", time.Now().UnixNano()%100000)
	}
