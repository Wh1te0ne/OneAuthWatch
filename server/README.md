...
[![Stars](https://img.shields.io/github/stars/onllm-dev/onwatch?style=for-the-badge&logo=github&logoColor=white&label=Stars&color=181717)](https://github.com/Wh1te0ne/OneAuthWatch/stargazers)
...
[![Downloads](https://img.shields.io/github/downloads/onllm-dev/onwatch/total?style=for-the-badge&logo=github&logoColor=white&label=Downloads&color=181717)](https://github.com/Wh1te0ne/OneAuthWatch/releases)  
...
[![Go Report Card](https://goreportcard.com/badge/github.com/Wh1te0ne/OneAuthWatch/server?style=for-the-badge)](https://goreportcard.com/report/github.com/Wh1te0ne/OneAuthWatch/server)

**Compatibility & Docs**

[![Version](https://img.shields.io/badge/Version-v2.11.31-0EA5E9?style=for-the-badge)](https://github.com/Wh1te0ne/OneAuthWatch/releases/tag/v2.11.31)
...
[![pkg.go.dev](https://img.shields.io/badge/pkg.go.dev-reference-007D9C?style=for-the-badge&logo=go&logoColor=white)](https://pkg.go.dev/github.com/Wh1te0ne/OneAuthWatch/server)
...
```bash
curl -fsSL https://raw.githubusercontent.com/Wh1te0ne/OneAuthWatch/main/server/install.sh | bash
```
...
Or download `install.bat` from the [Releases](https://github.com/Wh1te0ne/OneAuthWatch/releases) page and double-click it.
...
**Download binaries** from the [Releases](https://github.com/Wh1te0ne/OneAuthWatch/releases) page. Binaries are available for macOS (ARM64, AMD64), Linux (AMD64, ARM64), and Windows (AMD64).

**Or build from source** (requires Go 1.25+):

```bash
git clone https://github.com/Wh1te0ne/OneAuthWatch.git && cd OneAuthWatch/server
...
The Docker image uses a distroless base (~10-12 MB) and runs as non-root. An Alpine variant with shell access is also available (`ghcr.io/wh1te0ne/oneauthwatch:alpine`). Data persists via volume mount at `/data`. Logs go to stdout (`docker logs -f onwatch`). See [Docker Deployment](#docker-deployment) for details.
...
Menubar is currently in beta. Feedback is highly appreciated at [github.com/onllm-dev/onwatch/issues](https://github.com/Wh1te0ne/OneAuthWatch/issues).
...
| ------------ | ----------------------- | ---------------------------- | ----------------------------------- |
| `--interval` | `ONEAUTHWATCH_POLL_INTERVAL` | `120`                        | Poll interval in seconds (10--3600) |
| `--port`     | `ONEAUTHWATCH_PORT`          | `9211`                       | Dashboard HTTP port                 |
| `--db`       | `ONEAUTHWATCH_DB_PATH`       | `~/.onwatch/data/onwatch.db` | SQLite database path                |
...
| `ONEAUTHWATCH_ADMIN_USER`     | Optional dashboard auth username                       |
| `ONEAUTHWATCH_ADMIN_PASS`     | Optional dashboard auth password                       |
| `ONEAUTHWATCH_LOG_LEVEL`      | Log level: debug, info, warn, error                    |
| `ONEAUTHWATCH_HOST`           | Bind address (default: `0.0.0.0`)                      |
...
docker run -d --name onwatch -p 9211:9211 \
  ...
  ghcr.io/wh1te0ne/oneauthwatch:latest
```
...
docker run -d --name onwatch -p 9211:9211 \
  -v onwatch-data:/data \
  -e SYNTHETIC_API_KEY=your_key_here \
  ghcr.io/wh1te0ne/oneauthwatch:alpine
```
...
| `ONEAUTHWATCH_ADMIN_USER`    | Optional dashboard auth username           | --         |
| `ONEAUTHWATCH_ADMIN_PASS`    | Optional dashboard auth password           | --         |
| `ONEAUTHWATCH_POLL_INTERVAL` | Polling interval (seconds)                 | `120`      |
| `ONEAUTHWATCH_LOG_LEVEL`     | Log level                                  | `info`     |
...
**Debugging:** The default distroless image has no shell. Use the Alpine variant (`ghcr.io/wh1te0ne/oneauthwatch:alpine`) if you need `docker exec` access, or use a sidecar: `docker run -it --rm --pid=container:onwatch --net=container:onwatch nicolaka/netshoot bash`
