# ═══════════════════════════════════════════════════════════════════════
# OneAuthWatch Server Installer for Windows
# Usage: irm https://raw.githubusercontent.com/Wh1te0ne/OneAuthWatch/main/server/install.ps1 | iex
...
# ─── Configuration ─────────────────────────────────────────────────────
$INSTALL_DIR = if ($env:ONEAUTHWATCH_INSTALL_DIR) { $env:ONEAUTHWATCH_INSTALL_DIR } else { Join-Path $env:USERPROFILE ".oneauthwatch" }
$BIN_DIR = Join-Path $INSTALL_DIR "bin"
$DATA_DIR = Join-Path $INSTALL_DIR "data"
$REPO = "Wh1te0ne/OneAuthWatch"
$ASSET_NAME = "oneauthwatch-server-windows-amd64.exe"
...
function Get-AnthropicConfig {
    ...
    Write-Host "  ${DIM}OneAuthWatch can auto-detect your Claude Code credentials.${NC}"
    Write-Host ""
...
function Get-CodexConfig {
    ...
    Write-Host "  ${DIM}OneAuthWatch can auto-detect your Codex OAuth token from auth.json.${NC}"
    Write-Host ""
...
function Stop-ExistingInstance {
    $pidFile = Join-Path $INSTALL_DIR "oneauthwatch-server.pid"
    if (Test-Path $pidFile) {
        try {
            ...
            if ($content -match "^(\d+)") {
                ...
                if ($proc -and $proc.Name -like "*oneauthwatch-server*") {
    ...
    # Also try to stop any running oneauthwatch-server processes
    Get-Process -Name "oneauthwatch-server*" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
}
...
function Install-Binary {
    ...
    $dest = Join-Path $BIN_DIR "oneauthwatch-server.exe"
    $tempDest = Join-Path $env:TEMP "oneauthwatch-server-download-$PID.exe"

    Write-Info "Downloading oneauthwatch-server for ${BOLD}windows-amd64${NC}..."
    Write-Info "  URL:  $url"
    ...
    Write-Ok "Installed ${BOLD}oneauthwatch-server $version${NC}"
}

# ─── Create Wrapper Script ─────────────────────────────────────────────

function New-WrapperScript {
    $wrapperPath = Join-Path $INSTALL_DIR "oneauthwatch-server.cmd"
    $binaryPath = Join-Path $BIN_DIR "oneauthwatch-server.exe"

    @"
@echo off
cd /d "%USERPROFILE%\.oneauthwatch" 2>nul
...
function Start-InteractiveSetup {
    ...
    if (Test-Path $envFile) {
        ...
        if ($hasSyn -or $hasZai -or $hasAnth -or $hasCodex -or $hasAnti -or $hasGemini) {
            ...
            if ($envContent -match "ONEAUTHWATCH_PORT=(\d+)") {
                $script:SetupPort = $Matches[1]
            } else {
                $script:SetupPort = "9211"
            }
            $script:SetupUsername = ""
    ...
    # Dashboard Access
    Write-Host ""
    Write-Host "  ${BOLD}--- Dashboard Access ---${NC}"
    Write-Host ""
    Write-Host "  ${DIM}OneAuthWatch server auth is disabled by default.${NC}"
    Write-Host "  ${DIM}To enable it later, set ONEAUTHWATCH_ADMIN_USER and ONEAUTHWATCH_ADMIN_PASS manually.${NC}"
...
ONEAUTHWATCH_POLL_INTERVAL=$pollInterval

# Dashboard port
ONEAUTHWATCH_PORT=$($script:SetupPort)
"@
    ...
    Write-Host "  ${BOLD}|${NC}  Auth:      $("disabled".PadRight(29))${BOLD}|${NC}"
...
function Start-OneAuthWatch {
    ...
    Write-Info "Starting OneAuthWatch..."
    Write-Host ""
    ...
    try {
        ...
        if ($portListening) {
            Write-Ok "OneAuthWatch is running (listening on port $port)"
        } else {
            ...
            if ($running) {
                Write-Host "  ${YELLOW}!${NC} OneAuthWatch process started but port $port not yet available"
                Write-Host ""
                Write-Host "  ${BOLD}If you see a Windows Firewall popup:${NC}"
                Write-Host "    Click ${GREEN}'Allow access'${NC} to let OneAuthWatch accept connections"
                Write-Host ""
                ...
                while ($waited -lt 30) {
                    ...
                    if (Test-PortListening -Port ([int]$port)) {
                        Write-Host ""
                        Write-Ok "OneAuthWatch is now running (listening on port $port)"
                        $portListening = $true
                        break
                    }
                }
                ...
                if (-not $portListening) {
                    Write-Host ""
                    Write-Warn "OneAuthWatch started but couldn't verify it's listening on port $port"
                    Write-Host "  ${DIM}Try opening http://localhost:$port in your browser${NC}"
                }
            } else {
                Write-Host ""
                Write-Host "  ${RED}${BOLD}OneAuthWatch failed to start${NC}"
                Write-Host ""
                Write-Host "  Run in debug mode to see errors:"
                Write-Host "    ${CYAN}cd $INSTALL_DIR${NC}"
                Write-Host "    ${CYAN}.\bin\oneauthwatch-server.exe --debug${NC}"
                Pop-Location
                return
            }
        }
...
function Main {
    Write-Host ""
    Write-Host "  ${BOLD}OneAuthWatch Installer${NC}"
    ...
    Write-Host ""
    Write-Ok "Windows detected - OneAuthWatch runs as a background process"
    Write-Host "  ${DIM}Manage with:${NC}"
    Write-Host "    ${CYAN}oneauthwatch-server${NC}           # Start (runs in background)"
    Write-Host "    ${CYAN}oneauthwatch-server stop${NC}      # Stop"
    Write-Host "    ${CYAN}oneauthwatch-server status${NC}    # Status"
    Write-Host "    ${CYAN}oneauthwatch-server --debug${NC}   # Run in foreground (logs to stdout)"
