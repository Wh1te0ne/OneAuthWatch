#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════
# OneAuthWatch Server Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/Wh1te0ne/OneAuthWatch/main/server/install.sh | bash
# ═══════════════════════════════════════════════════════════════════════
set -euo pipefail

INSTALL_DIR="${ONEAUTHWATCH_INSTALL_DIR:-$HOME/.oneauthwatch}"
BIN_DIR="${INSTALL_DIR}/bin"
REPO="Wh1te0ne/OneAuthWatch"
SERVICE_NAME="oneauthwatch"
...
interactive_setup() {
    ...
    if [[ -f "$env_file" ]]; then
        ...
        SETUP_PORT="$(env_get ONEAUTHWATCH_PORT)"
    ...
    # ── Dashboard Access ──
    printf "\n  ${BOLD}━━━ Dashboard Access ━━━${NC}\n\n"
    printf "  ${DIM}OneAuthWatch server auth is disabled by default.${NC}\n"
    printf "  ${DIM}To enable it later, set ONEAUTHWATCH_ADMIN_USER and ONEAUTHWATCH_ADMIN_PASS manually.${NC}\n"
    ...
    # ── Write .env ──
    {
        ...
        echo "# Optional dashboard auth (disabled by default)"
        echo "# ONEAUTHWATCH_ADMIN_USER="
        echo "# ONEAUTHWATCH_ADMIN_PASS="
        echo ""
        echo "# Polling interval in seconds (10-3600)"
        echo "ONEAUTHWATCH_POLL_INTERVAL=${poll_interval}"
        echo ""
        echo "# Dashboard port"
        echo "ONEAUTHWATCH_PORT=${SETUP_PORT}"
    } > "$env_file"
    ...
    printf "  ${BOLD}│${NC}  Auth:      %-29s${BOLD}│${NC}\n" "disabled"
...
print_errors() {
    ...
    printf "     Change ONEAUTHWATCH_PORT in ${CYAN}${INSTALL_DIR}/.env${NC}\n"
