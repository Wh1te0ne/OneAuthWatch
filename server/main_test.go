...
func TestHasFlagAndHasCommand(t *testing.T) {
	...
	os.Args = []string{"oneauthwatch-server", "--debug", "status", "--test"}
...
func TestDeriveEncryptionKey_UsesEncryptionSalt(t *testing.T) {
	passwordHash := sha256hex("oneauthwatch-server-admin")
...
func TestStatusLogCandidates(t *testing.T) {
	t.Run("prefers db directory then home then cwd", func(t *testing.T) {
		...
		dbPath := filepath.Join(t.TempDir(), "data", "oneauthwatch.db")
		...
		want := []string{
			...
			filepath.Join(homeDir, ".oneauthwatch", "main.log"),
			filepath.Join(homeDir, ".oneauthwatch", "menubar.log"),
...
func TestWriteEnvFile(t *testing.T) {
	...
	checks := []string{
		...
		"# Generated by 'oneauthwatch-server setup' on ",
...
func TestGeneratePassword_Format(t *testing.T) {
	...
	if strings.HasPrefix(got, "oneauthwatch-server") {
		if !regexp.MustCompile(`^oneauthwatch-server\d{1,5}$`).MatchString(got) {
