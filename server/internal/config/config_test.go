...
func TestConfig_LoadsFromEnv(t *testing.T) {
	...
	os.Setenv("ONEAUTHWATCH_POLL_INTERVAL", "120")
	os.Setenv("ONEAUTHWATCH_PORT", "8080")
	os.Setenv("ONEAUTHWATCH_ADMIN_USER", "myuser")
	os.Setenv("ONEAUTHWATCH_ADMIN_PASS", "mypass")
	os.Setenv("ONEAUTHWATCH_DB_PATH", "/tmp/test.db")
	os.Setenv("ONEAUTHWATCH_LOG_LEVEL", "debug")
	defer os.Clearenv()
...
func TestConfig_ValidatesInterval_Minimum(t *testing.T) {
	os.Setenv("ZAI_API_KEY", "zai_test_key")
	os.Setenv("ONEAUTHWATCH_POLL_INTERVAL", "5")
	defer os.Clearenv()
...
func TestConfig_ValidatesInterval_Maximum(t *testing.T) {
	os.Setenv("ZAI_API_KEY", "zai_test_key")
	os.Setenv("ONEAUTHWATCH_POLL_INTERVAL", "7200")
	defer os.Clearenv()
...
func TestConfig_ValidatesPort_Range(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			os.Clearenv()
			os.Setenv("ZAI_API_KEY", "zai_test_key")
			os.Setenv("ONEAUTHWATCH_PORT", tt.port)
			defer os.Clearenv()
...
func TestConfig_LoadWithArgs_FlagOverridesEnv(t *testing.T) {
	...
	os.Setenv("ONEAUTHWATCH_POLL_INTERVAL", "120")
	os.Setenv("ONEAUTHWATCH_PORT", "8080")
	os.Setenv("ONEAUTHWATCH_DB_PATH", "/tmp/env.db")
	defer os.Clearenv()
...
func TestConfig_SecureCookiesFromEnv(t *testing.T) {
	os.Setenv("SYNTHETIC_API_KEY", "syn_test_key")
	os.Setenv("ONEAUTHWATCH_SECURE_COOKIES", "true")
	defer os.Clearenv()
	...
	if !cfg.SecureCookies {
		t.Error("SecureCookies should be true when ONEAUTHWATCH_SECURE_COOKIES=true")
	}
}

func TestConfig_SessionIdleTimeoutFromEnv(t *testing.T) {
	os.Setenv("SYNTHETIC_API_KEY", "syn_test_key")
	os.Setenv("ONEAUTHWATCH_SESSION_IDLE_TIMEOUT", "300")
	defer os.Clearenv()
...
func TestIsOnwatchEnvFile_WithOnwatchKeys(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "contains ONEAUTHWATCH_ prefix",
			content: "ONEAUTHWATCH_PORT=9211\nONEAUTHWATCH_ADMIN_USER=admin\n",
			want:    true,
		},
		{
...
func TestLoadEnvFile_PrefersStandardLocation(t *testing.T) {
	...
	if got := os.Getenv("ONEAUTHWATCH_PORT"); got != "9999" {
		t.Errorf("ONEAUTHWATCH_PORT = %q, want %q", got, "9999")
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			origKeys := []string{
				...
				"ONEAUTHWATCH_POLL_INTERVAL", "ONEAUTHWATCH_PORT", "ONEAUTHWATCH_DB_PATH",
				"ONEAUTHWATCH_ADMIN_USER", "ONEAUTHWATCH_ADMIN_PASS",
