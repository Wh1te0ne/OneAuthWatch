// Package config handles loading and validation of OneAuthWatch configuration.
...
type Config struct {
	...
	PollInterval       time.Duration // ONEAUTHWATCH_POLL_INTERVAL (seconds → Duration)
	Port               int           // ONEAUTHWATCH_PORT
	Host               string        // ONEAUTHWATCH_HOST (bind address, default: 0.0.0.0)
	SecureCookies      bool          // ONEAUTHWATCH_SECURE_COOKIES (set Secure flag on cookies)
	AdminUser          string        // ONEAUTHWATCH_ADMIN_USER
	AdminPass          string        // ONEAUTHWATCH_ADMIN_PASS
	AdminPassHash      string        // SHA-256 hash of password (set after DB check)
	DBPath             string        // ONEAUTHWATCH_DB_PATH
	DBPathExplicit     bool          // true if user explicitly set --db or ONEAUTHWATCH_DB_PATH
	LogLevel           string        // ONEAUTHWATCH_LOG_LEVEL
	SessionIdleTimeout time.Duration // ONEAUTHWATCH_SESSION_IDLE_TIMEOUT (seconds → Duration)
	BasePath           string        // ONEAUTHWATCH_BASE_PATH (subdirectory hosting, e.g. "/oneauthwatch")
...
// env reads a single environment variable.
...
// appEnvKeys are the keys that identify a OneAuthWatch-compatible .env file.
...
// isAppEnvFile checks if a file contains app-specific configuration.
func isAppEnvFile(path string) bool {
...
func loadEnvFile() {
	...
	if home, err := os.UserHomeDir(); err == nil {
		standardPath := filepath.Join(home, ".oneauthwatch", ".env")
	...
	// Fallback to current directory .env - only if it's app-specific
	localPath := ".env"
	if _, err := os.Stat(localPath); err == nil {
		if isAppEnvFile(localPath) {
...
func loadFromEnvAndFlags(flags *flagValues) (*Config, error) {
	...
	} else if env := env("ONEAUTHWATCH_POLL_INTERVAL"); env != "" {
	...
	if flags.port > 0 {
		cfg.Port = flags.port
	} else if env := env("ONEAUTHWATCH_PORT"); env != "" {
		if v, err := strconv.Atoi(env); err == nil {
			cfg.Port = v
		}
	}
	...
	// Admin credentials
	cfg.AdminUser = env("ONEAUTHWATCH_ADMIN_USER")
	cfg.AdminPass = env("ONEAUTHWATCH_ADMIN_PASS")

	// DB Path
	if flags.db != "" {
		cfg.DBPath = expandTilde(flags.db)
		cfg.DBPathExplicit = true
	} else if envDB := env("ONEAUTHWATCH_DB_PATH"); envDB != "" {
		cfg.DBPath = expandTilde(envDB)
		cfg.DBPathExplicit = true
	}

	// Log Level
	cfg.LogLevel = env("ONEAUTHWATCH_LOG_LEVEL")

	// Host (bind address)
	cfg.Host = env("ONEAUTHWATCH_HOST")

	// Secure Cookies
	if env := env("ONEAUTHWATCH_SECURE_COOKIES"); env != "" {
		cfg.SecureCookies = strings.ToLower(env) == "true" || env == "1"
	}

	// Base Path (subdirectory hosting, e.g. "/oneauthwatch")
	cfg.BasePath = strings.TrimSpace(env("ONEAUTHWATCH_BASE_PATH"))

	// Session Idle Timeout (seconds)
	if env := env("ONEAUTHWATCH_SESSION_IDLE_TIMEOUT"); env != "" {
		if v, err := strconv.Atoi(env); err == nil {
...
func (c *Config) applyDefaults() {
	...
	if c.DBPath == "" {
		// Check if running in Docker and use /data/oneauthwatch.db as default
		if c.IsDockerEnvironment() {
			c.DBPath = "/data/oneauthwatch.db"
		} else {
			home, err := os.UserHomeDir()
			if err != nil || home == "" {
				c.DBPath = "./oneauthwatch.db"
			} else {
				c.DBPath = filepath.Join(home, ".oneauthwatch", "data", "oneauthwatch.db")
			}
		}
	}
	if c.LogLevel == "" {
...
// --debug:      returns .oneauthwatch.log file (stdout gets only warn/error via separate handler)
// Docker:       returns os.Stdout (containers should log to stdout)
// Background:   returns .oneauthwatch.log file
...
// HasAdminAuth returns true when server-side password auth is explicitly enabled.
func (c *Config) HasAdminAuth() bool {
	return strings.TrimSpace(c.AdminUser) != "" && strings.TrimSpace(c.AdminPass) != ""
}

// IsDefaultPassword returns true if the default password "changeme" is being used.
func (c *Config) IsDefaultPassword() bool {
	return c.HasAdminAuth() && c.AdminPass == "changeme"
}
