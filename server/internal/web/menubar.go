...
func (h *Handler) MenubarSummary(w http.ResponseWriter, r *http.Request) {
	if !menubar.IsSupported() && os.Getenv("ONEAUTHWATCH_TEST_MODE") != "1" {
		http.NotFound(w, r)
		return
	}
...
func (h *Handler) MenubarTest(w http.ResponseWriter, r *http.Request) {
	if os.Getenv("ONEAUTHWATCH_TEST_MODE") != "1" {
		http.NotFound(w, r)
		return
	}
	settings, _ := h.menubarSettings()
...
func (h *Handler) renderMenubarHTML(view menubar.ViewType, settings *menubar.Settings) (string, error) {
	...
	html := strings.Replace(string(page), "__ONEAUTHWATCH_MENUBAR_BOOTSTRAP__", string(bootstrap), 1)
	version := strings.TrimSpace(h.version)
	if version == "" {
		version = "dev"
	}
	return strings.Replace(html, "__ONEAUTHWATCH_MENUBAR_VERSION__", version, 1), nil
}
