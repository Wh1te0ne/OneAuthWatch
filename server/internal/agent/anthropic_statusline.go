...
// oneAuthWatchDataDir returns the OneAuthWatch data directory path.
func oneAuthWatchDataDir() string {
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return ""
	}
	return filepath.Join(home, ".oneauthwatch", "data")
}
...
func hasBridgeSnippet(command string) bool {
	return strings.Contains(command, bridgeSnippet)
}
...
func removeBridgeSnippet(command string) string {
	for _, snippet := range []string{bridgeSnippet, legacyBridgeSnippet} {
		if idx := strings.Index(command, snippet+" | "); idx == 0 {
			return strings.TrimSpace(command[len(snippet+" | "):])
		}
		if command == snippet+" > /dev/null" {
			return ""
		}
	}
	// Not our command
...
func writeClaudeSettings(settings map[string]interface{}) error {
	...
	tmpFile, err := os.CreateTemp(dir, ".oneauthwatch-settings-*.tmp")
	if err != nil {
...
func SetupStatuslineBridge(logger *slog.Logger) error {
	...
	if hasLegacyBridgeSnippet(currentCmd) {
		currentCmd = removeBridgeSnippet(currentCmd)
	}
...
func DisableStatuslineBridge(logger *slog.Logger) error {
	...
	currentCmd := getCurrentStatusLineCommand(settings)
	if !hasBridgeSnippet(currentCmd) && !hasLegacyBridgeSnippet(currentCmd) {
