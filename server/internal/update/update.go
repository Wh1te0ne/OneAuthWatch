...
const (
	githubReleasesURL    = "https://api.github.com/repos/Wh1te0ne/OneAuthWatch/releases/latest"
	downloadBaseURL      = "https://github.com/Wh1te0ne/OneAuthWatch/releases/download"
...
func (u *Updater) Check() (UpdateInfo, error) {
	...
	req.Header.Set("User-Agent", "oneauthwatch/"+u.currentVersion)
...
func (u *Updater) Apply() error {
	...
	tmpFile, err := os.CreateTemp(exeDir, "oneauthwatch.tmp.*")
	if err != nil {
...
// Falls back to "oneauthwatch.service" if detection fails.
func DetectServiceName() string {
	data, err := readCgroupFile()
	if err == nil {
		for _, line := range strings.Split(string(data), "\n") {
			// cgroup v2 line: "0::/system.slice/oneauthwatch.service"
...
// restart method fails. Tries the detected service name first, then "oneauthwatch.service".
func (u *Updater) fallbackSystemctlRestart() error {
	for _, name := range []string{DetectServiceName(), "oneauthwatch.service"} {
...
func (u *Updater) binaryDownloadURL(version string) string {
	name := fmt.Sprintf("oneauthwatch-server-%s-%s", runtime.GOOS, runtime.GOARCH)
...
func checkWritable(dir string) error {
	f, err := os.CreateTemp(dir, ".oneauthwatch-write-test-*")
	if err != nil {
		return err
	}
	name := f.Name()
