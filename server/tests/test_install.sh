...
setup() {
    TEST_DIR=$(mktemp -d)
    export ONEAUTHWATCH_INSTALL_DIR="$TEST_DIR"
...
test_synthetic_only() {
    ...
    assert_eq "ONEAUTHWATCH_ADMIN_USER" "admin" && \
    assert_eq "ONEAUTHWATCH_PORT" "9211"
}
...
test_custom_port_and_interval() {
    setup && source_functions
    run_setup "1\nsyn_custom_123\nmyuser\nmypass\n8080\n30\n"
    assert_eq "ONEAUTHWATCH_PORT" "8080" && \
    assert_eq "ONEAUTHWATCH_POLL_INTERVAL" "30" && \
    assert_eq "ONEAUTHWATCH_ADMIN_USER" "myuser"
}
...
ONEAUTHWATCH_ADMIN_PASS=existingpass
