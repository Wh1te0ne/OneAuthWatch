...
const DEFAULT_SERVER_URL = 'http://127.0.0.1:9211';
const WORKSPACE_METADATA_REFRESH_INTERVAL_MS = 60_000;
...
function normalizeProvider(value: unknown): AccountProvider {
  switch (value) {
    case 'claude':
    case 'gemini':
    case 'codex':
...
function normalizeStoredAccount(account: StoredAccount): StoredAccount {
  return {
    ...account,
    provider: normalizeProvider((account as Partial<StoredAccount>).provider),
  };
}
...
function decodeTokenPayload(token?: string | null): Record<string, unknown> | null {
  ...
  try {
    ...
    console.warn('Failed to decode provider token payload:', error);
    return null;
  }
}
...
function mapProviderPlanType(value?: string | null): AccountInfo['planType'] {
...
function buildAutoAlias(accountInfo: AccountInfo): string {
  const emailPrefix = accountInfo.email.split('@')[0]?.trim();
  if (emailPrefix) {
    return emailPrefix;
  }
  const workspaceName = (accountInfo.workspaceName ?? '').trim();
  if (workspaceName) {
...
function getProviderMatchRank(
  ...
  if (provider === 'codex') {
    return getMatchRank(a, b);
  }

  if (a.userId && b.userId && a.userId === b.userId) return 4;
  if (a.email && b.email && a.email === b.email) return 3;
  if (a.accountId && b.accountId && a.accountId === b.accountId) return 2;
  return 0;
}
...
function findBestProviderMatch(
  accounts: StoredAccount[],
  provider: AccountProvider,
  identity: AccountIdentity
): { index: number; rank: number; count: number } {
  ...
  accounts.forEach((account, index) => {
    ...
    const rank = getProviderMatchRank(
      provider,
      buildIdentityFromAccountInfo(account.accountInfo),
      identity
    );
    if (rank === 0) return;
    if (rank > bestRank) {
...
async function saveAccountAuth(accountId: string, authConfig: unknown): Promise<void> {
...
export async function loadServerAccountsStore(serverUrl: string): Promise<AccountsStore> {
  ...
  return {
    ...
    accounts: Array.isArray(parsed.accounts)
      ? parsed.accounts.map((account) => normalizeStoredAccount(account as StoredAccount))
      : [],
...
export async function addAccount(
  ...
  if (existingIndex >= 0) {
    ...
    const workspaceMetadata = await fetchWorkspaceMetadata(existingAccount.id);
  ...
  const workspaceMetadata = await fetchWorkspaceMetadata(newAccount.id);
...
async function upsertProviderAccount(
  ...
  const store = await loadAccountsStore();
  const identity = buildIdentityFromAccountInfo(accountInfo);
  const match = findBestProviderMatch(store.accounts, provider, identity);
  const existingIndex = match.rank >= 2 ? match.index : -1;
  const now = new Date().toISOString();
  ...
  if (existingIndex >= 0) {
    ...
    await saveAccountAuth(existingAccount.id, authPayload);
    store.accounts[existingIndex] = updatedAccount;
    await saveAccountsStore(store);
  ...
  const newAccount: StoredAccount = {
    id: generateId(),
    provider,
    alias: alias || buildAutoAlias(accountInfo),
    accountInfo,
    isActive: false,
    createdAt: now,
    updatedAt: now,
  };
  await saveAccountAuth(newAccount.id, authPayload);
  store.accounts.push(newAccount);
  await saveAccountsStore(store);
  return newAccount;
}

async function resolveClaudeAccountFile(homeDir: string): Promise<ClaudeAccountFile | null> {
  return readOptionalJsonFile<ClaudeAccountFile>(`${homeDir}\\.claude.json`);
}

export async function syncCurrentClaudeAccount(): Promise<boolean> {
  try {
    const homeDir = await invoke<string>('get_home_dir');
    const credentialsPath = `${homeDir}\\.claude\\.credentials.json`;
    const credentials = await readOptionalJsonFile<ClaudeCredentialsFile>(credentialsPath);
    const accessToken = credentials?.claudeAiOauth?.accessToken?.trim();
    if (!accessToken) {
      return false;
    }

    const accountFile = await resolveClaudeAccountFile(homeDir);
    const email =
      normalizeEmail(accountFile?.oauthAccount?.emailAddress)
      ?? normalizeEmail((credentials as Record<string, unknown> | undefined)?.['emailAddress'] as string | undefined);
    const accountUuid = normalizeId(accountFile?.oauthAccount?.accountUuid);
    const organizationName = normalizeId(accountFile?.oauthAccount?.organizationName);
    ...
    const accountInfo: AccountInfo = {
      email: email ?? `claude-${accountUuid ?? 'account'}@local.invalid`,
      planType: mapProviderPlanType(credentials?.claudeAiOauth?.subscriptionType),
      accountId: accountUuid ?? email ?? 'claude-account',
      userId: accountUuid ?? email ?? 'claude-user',
      accountStructure: 'personal',
      workspaceName: organizationName ?? null,
      organizations: [],
    };

    await upsertProviderAccount('claude', accountInfo, credentials, undefined);
    return true;
  } catch (error) {
    console.warn('Failed to sync current Claude Code account:', error);
    return false;
  }
}

export async function syncCurrentGeminiAccount(): Promise<boolean> {
  try {
    const homeDir = await invoke<string>('get_home_dir');
    const credentialsPath = `${homeDir}\\.gemini\\oauth_creds.json`;
    const credentials = await readOptionalJsonFile<GeminiCredentialsFile>(credentialsPath);
    if (!credentials?.refresh_token && !credentials?.access_token) {
      return false;
    }

    const idTokenPayload = decodeTokenPayload(credentials.id_token);
    const email = normalizeEmail(readStringField(idTokenPayload?.email));
    const subject = normalizeId(readStringField(idTokenPayload?.sub));
    const name = normalizeId(readStringField(idTokenPayload?.name));
    ...
    const accountInfo: AccountInfo = {
      email: email ?? `gemini-${subject ?? 'account'}@local.invalid`,
      planType: 'free',
      accountId: subject ?? email ?? 'gemini-account',
      userId: subject ?? email ?? 'gemini-user',
      accountStructure: 'personal',
      workspaceName: name ?? null,
      organizations: [],
    };

    await upsertProviderAccount('gemini', accountInfo, credentials, undefined);
    return true;
  } catch (error) {
    console.warn('Failed to sync current Gemini account:', error);
    return false;
  }
}

export async function refreshAccountsWorkspaceMetadata(): Promise<StoredAccount[]> {
  const now = Date.now();
  if (workspaceMetadataRefreshPromise) {
    return workspaceMetadataRefreshPromise;
  }
  if (now-lastWorkspaceMetadataRefreshAt < WORKSPACE_METADATA_REFRESH_INTERVAL_MS) {
    const store = await loadAccountsStore();
    return store.accounts;
  }

  workspaceMetadataRefreshPromise = (async () => {
  const store = await loadAccountsStore();
  let changed = false;
  ...
  const updatedAccounts = await Promise.all(
    store.accounts.map(async (account) => {
      ...
      const metadata = await fetchWorkspaceMetadata(account.id);
  ...
  if (changed) {
    ...
    lastWorkspaceMetadataRefreshAt = Date.now();
    return changed ? updatedAccounts : store.accounts;
  })();

  try {
    return await workspaceMetadataRefreshPromise;
  } finally {
