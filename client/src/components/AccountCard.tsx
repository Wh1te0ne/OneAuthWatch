...
const planTypeColors: Record<string, string> = {
  ...
  plus: 'bg-stone-100 text-stone-700',
  pro: 'bg-stone-200 text-stone-700',
  team: 'bg-stone-100 text-stone-700',
};
...
const accountErrorStates: Record<string, { label: string; className: string }> = {
  expired: { label: '已过期', className: 'bg-red-50 text-red-700 border-red-200' },
  forbidden: { label: '已封号', className: 'bg-red-50 text-red-700 border-red-200' },
  error: { label: '异常', className: 'bg-red-50 text-red-700 border-red-200' },
};

const providerLimitLabels: Record<StoredAccount['provider'], { fiveHour: string; weekly: string }> = {
  codex: { fiveHour: '5h 限额', weekly: '周限额' },
  claude: { fiveHour: '5h 限额', weekly: '7天限额' },
  gemini: { fiveHour: 'Pro 配额', weekly: 'Flash 配额' },
};
...
export const AccountCard: React.FC<AccountCardProps> = ({
  ...
  const providerLabels = providerLimitLabels[account.provider ?? 'codex'];
  ...
  const fiveHourLabel = usageInfo?.fiveHourLimitLabel || providerLabels.fiveHour;
  const weeklyLabel = usageInfo?.weeklyLimitLabel || providerLabels.weekly;
  ...
  const avatarClass = isErrorAccount
    ? 'w-11 h-11 rounded-2xl bg-red-100 text-red-700 flex items-center justify-center font-semibold'
    ...
    <div className={cardClass}>
      <div className="account-card__header">
        ...
        <div className="account-card__actions shrink-0">
          {allowActions && isDesktopMode && !isActive && (
            <button
              onClick={onSwitch}
              className="h-8 px-3 bg-[var(--dash-accent)] text-white rounded-full text-xs font-medium hover:opacity-92 transition-colors whitespace-nowrap"
            >
              切换
            </button>
          )}
          {allowActions && isDesktopMode && (
            <>
              <button
                onClick={onRefresh}
                disabled={isRefreshing}
                className="h-8 w-8 rounded-full border border-[var(--dash-border)] text-[var(--dash-text-secondary)] hover:text-[var(--dash-text-primary)] hover:border-stone-300 transition-colors flex items-center justify-center disabled:opacity-50 disabled:pointer-events-none"
                title="刷新用量"
              >
                <svg
      ...
      <div className="space-y-3">
        <div className="space-y-2">
          <div className="flex items-baseline gap-2">
            <span className="text-xs text-[var(--dash-text-muted)]">{fiveHourLabel}</span>
            {fiveHourResetText && (
          ...
          {hasUsage ? (
            <UsageBar label={fiveHourLabel} showLabel={false} percentLeft={fiveHourLeft ?? 0} />
          ) : (
        ...
        <div className="space-y-2">
          <div className="flex items-baseline gap-2 min-w-0 flex-nowrap">
            <span className="text-xs text-[var(--dash-text-muted)] shrink-0">{weeklyLabel}</span>
            {weeklyResetText && (
          ...
          {hasUsage ? (
            <UsageBar label={weeklyLabel} showLabel={false} percentLeft={weeklyLeft ?? 0} />
          ) : (
            <div className="h-1.5 bg-stone-100 rounded-full" />
          )}
        </div>

        {(account.provider === 'codex' || accountInfo.subscriptionActiveUntil) && (
          <div className="space-y-2">
          <div className="flex items-baseline gap-2 min-w-0 flex-nowrap">
