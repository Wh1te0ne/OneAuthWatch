...
export const StatsSummary: React.FC<StatsSummaryProps> = ({ accounts, embedded = false }) => {
  ...
  const content = (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:divide-x md:divide-stone-200">
      <div className="md:pr-4">
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <span className="dash-pill bg-[var(--dash-accent-soft)] text-[var(--dash-text-primary)]">
            活跃 {activeCount}
          </span>
        </div>
        <div className="mt-3 flex gap-2 flex-wrap">
          {Object.entries(planCounts).map(([plan, count]) => (
            <span
              key={plan}
              className="text-xs px-2 py-1 rounded-full bg-stone-100 text-[var(--dash-text-secondary)]"
            >
      ...
      <div className="md:px-4">
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <span className={`dash-pill ${avgWeeklyLeft >= 50 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
      ...
      <div className="md:px-4">
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <span className={`dash-pill ${avgFiveHourLeft >= 50 ? 'bg-stone-100 text-stone-700' : 'bg-rose-50 text-rose-700'}`}>
